...
For these situations, the `cubble` package offers functions such as `make_cubble()` to help users structure their data into `cubble` objects, optimised for glyph maps. This structuring facilitates detailed, insightful spatiotemporal visualizations, and seamless integration into the `sugarglider` package.
...
Tooltips must be provided in the `tooltip` argument's asthetic. The user can then plot their desired glyph map and save it as a ggplot object. This ggplot object is converted into an interactive glyph map using the `girafe()` function. 
